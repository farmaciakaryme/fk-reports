...
const TESTS_CONFIG = {
  ...
  ALCOHOLIMETRO: {
    ...
    fields: [
      ...
      { key: 'resultado', label: 'Resultado', type: 'computed', compute: (data) => parseFloat(data.gradosAlcohol || 0) > 0.1 ? 'POSITIVA' : 'NEGATIVA' }
    ],
...
const ReportGenerator = ({ onBack, pruebaData }) => {
  ...
  const handleInputChange = useCallback((field, value) => {
    setFormData(prev => {
      ...
      testConfig.fields.forEach(f => {
        if (f.type === 'computed' && f.compute) {
          newData[f.key] = f.compute(newData);
        }
      });
    ...
    try {
      ...
      if (pruebaData?.subPruebas && pruebaData.subPruebas.length > 0) {
        resultados = testConfig.fields
          .filter(f => f.type !== 'computed' && formData[f.key])
          .map((field, index) => ({
